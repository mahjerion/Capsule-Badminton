...
public class DubP2Move : MonoBehaviour
{
    ...
    public Transform p4DoublesPosition;
    ...
    public static int botLastShotType;
    ...
    private void FixedUpdate()
    {
        ...
        if (strokePossible == 1 && shuttle.position.x > 0f && points.GetComponent<Points>().lastHitter != "Player4")
        {
            if (botShot)
            {
                EnergyFirstPress();
    ...
    void MoveBot()
    {
        if (((points.GetComponent<Points>().lastHitter == "Player1" || 
            points.GetComponent<Points>().lastHitter == "Player3") && ThisBotIsCloser() && Points.doublesReceiver != 4)
    ...
    void CheckLastShotType()
    {
        ...
        if (botLastShotType == 0)
        {
            if (transform.position.z > p4Position.position.z)
            {
                doublesPosition.position = basePosition[2].position;
                p4DoublesPosition.position = basePosition[3].position;
            }
            else
            {
                doublesPosition.position = basePosition[3].position;
                p4DoublesPosition.position = basePosition[2].position;
            }
        }
        else if (transform.position.x < p4Position.position.x)
        {
            doublesPosition.position = basePosition[0].position;
            p4DoublesPosition.position = basePosition[1].position;
        }
        else
        {
            doublesPosition.position = basePosition[1].position;
            p4DoublesPosition.position = basePosition[0].position;
        }
    }
    ...
    bool ThisBotIsCloser()
    {
        ...
        float distanceThem = Vector3.Distance(p4Position.transform.position, distanceToShuttle);
