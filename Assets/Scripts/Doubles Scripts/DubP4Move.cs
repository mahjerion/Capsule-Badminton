...
public class DubP4Move : MonoBehaviour
{
    ...
    public Transform p2DoublesPosition;
    // used to compare p4 to p2 position
    ...
    void Update()
    {
        ...
        {
            // If the game hasn't started yet and p2 serves then:
            if (Points.gameStart == false && Points.server == 4)
            {
                if (BotMovement.amIABot == false)
                {
    ...
    private void FixedUpdate()
    {
        ...
        if (strokePossible == 1 && shuttle.position.x > 0f && points.GetComponent<Points>().lastHitter != "Player2")
        {
            if (botShot)
            {
                EnergyFirstPress();
    ...
    {
        if (points.GetComponent<Points>().lastHitter != "Player4" 
            && Points.doublesReceiver != 2)
        {
            PlayAnimation();
            EnergyOnHit();
            ShuttleContact();
    ...
    public Vector3 CheckHitDirection()
    {
        ...
        DubP2Move.xTargetHitTo = targets[targetX].position.x + ((shuttle.position.x - transform.position.x) * newInacc * accMod);
        DubP2Move.zTargetHitTo = targets[targetX].position.z + ((shuttle.position.z - transform.position.z) * newInacc * 2 * accMod);
    ...
    private void ShuttleContact()
    {
        ...
        // checks last shot type
        botJumpChanceRolled = false;
        CheckLastShotType();
        // checks for double hits
        Points.bluHits++;
        Points.redHits = 0;
    }
    ...
    private void CharacterSelect()
    {
        ...
        {
            BotDifficulty(BotMovement.difficultyLevel);
        }
    }
    ...
    // this bot is faster and more accurate
    void BotDifficulty(int levelOfBot)
    {
        speed = Mathf.Min(2.7f + 0.25f * levelOfBot);
    ...
    void MoveBot()
    {
        if (((points.GetComponent<Points>().lastHitter == "Player1" ||
            points.GetComponent<Points>().lastHitter == "Player3") && ThisBotIsCloser() && Points.doublesReceiver != 2)
            || Points.doublesReceiver == 4)
        {
            ...
            else
            {
                ...
                if (delayStarted == true)
                {
                    if (DubP1Move.xTargetHitTo <= 3)
                    {
                        moveDirection = new Vector3(DubP1Move.xTargetHitTo * 0.85f - transform.position.x,
                            0, DubP1Move.zTargetHitTo * 0.95f - transform.position.z);
                    }
                    else
                    {
                        moveDirection = new Vector3(DubP1Move.xTargetHitTo * 0.8f - transform.position.x,
                            0, DubP1Move.zTargetHitTo * 0.9f - transform.position.z);
                    }
                }
                else
                {
        ...
        if ((points.GetComponent<Points>().lastHitter == "Player1" ||
            points.GetComponent<Points>().lastHitter == "Player3") && BotMovement.difficultyLevel >= 1)
        {
            // Checks to see if shuttle is on top of bot, if it is then jump
            if (Mathf.Abs(shuttle.position.x - transform.position.x) <= (0.5f + BotMovement.difficultyLevel / 2)
                && Mathf.Abs(shuttle.position.y - transform.position.y) >= 3.5f
                && Mathf.Abs(shuttle.position.z - transform.position.z) <= (0.5f + BotMovement.difficultyLevel / 2)
    ...
    void CheckLastShotType()
    {
        ...
        {
            DubP2Move.botLastShotType = 0;
        }
        else
        {
            DubP2Move.botLastShotType = 1;
        }

        if (DubP2Move.botLastShotType == 0)
        {
            if (transform.position.z > p2Position.position.z)
            {
                doublesPosition.position = basePosition[2].position;
                p2DoublesPosition.position = basePosition[3].position;
            }
            else
            {
                doublesPosition.position = basePosition[3].position;
                p2DoublesPosition.position = basePosition[2].position;
            }
        }
        else if (transform.position.x < p2Position.position.x)
        {
            doublesPosition.position = basePosition[0].position;
            p2DoublesPosition.position = basePosition[1].position;
        }
        else
        {
            doublesPosition.position = basePosition[1].position;
            p2DoublesPosition.position = basePosition[0].position;
        }
    }
    ...
    bool ThisBotIsCloser()
    {
        ...
        distanceToShuttle.x = DubP1Move.xTargetHitTo;
        distanceToShuttle.y = 0f;
        distanceToShuttle.z = DubP1Move.zTargetHitTo;

        float distanceThis = Vector3.Distance(transform.position, distanceToShuttle);
        float distanceThem = Vector3.Distance(p2Position.transform.position, distanceToShuttle);
