...
public class DubP4Move : MonoBehaviour
{
    ...
    public Vector3 CheckHitDirection()
    {
        ...
        DubP2Move.xTargetHitTo = targets[targetX].position.x + ((shuttle.position.x - transform.position.x) * newXInacc * accMod);
        DubP2Move.zTargetHitTo = targets[targetX].position.z + ((shuttle.position.z - transform.position.z) * newZInacc * 2 * accMod);
